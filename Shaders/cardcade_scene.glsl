...
v4 GetDecalColor(decal_shader_data Decal, v3 CurrentPosition)
{
	...
	{
		Result = texture(DecalTexture, DecalCoords.xy);
	}
...
void main()
{
	f32 FragDepth = gl_FragCoord.z/(gl_FragCoord.w*FarPlane);
	f32 PreviousDepth = texelFetch(PrevDepthTexture, V2i(gl_FragCoord.xy), 0).r;
	if(PreviousDepth >= FragDepth)
	{
		discard;
	}
	else
