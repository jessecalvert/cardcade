...
// {
// 	v2 TexelSize = 1.0f/textureSize(Texture, LOD);
// 	v2 Offset = TexelSize*(k + 0.5f);
// 	v4 Sample0 = textureLod(Texture, TexCoord + V2(Offset.x, Offset.y), f32(LOD));
// 	v4 Sample1 = textureLod(Texture, TexCoord + V2(-Offset.x, Offset.y), f32(LOD));
// 	v4 Sample2 = textureLod(Texture, TexCoord + V2(Offset.x, -Offset.y), f32(LOD));
// 	v4 Sample3 = textureLod(Texture, TexCoord + V2(-Offset.x, -Offset.y), f32(LOD));
...
void main()
{
	FragColor = KawaseBlurSample(TexCoordsToFragment, Kernel).rgb;
}
