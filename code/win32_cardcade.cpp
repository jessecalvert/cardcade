/*@H
* File: win32_cardcade.cpp
* Author: Jesse Calvert
* Created: October 6, 2017, 17:42
* Last modified: December 17, 2018, 15:42
*/
...
{
	char *Filename = "w:/cardcade/build/loop_data_0.sld";
	return Filename;
}
...
Win32RegisterHotloads(win32_hotloader *Hotloader)
{
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_header.glsl", FileFlag_Header, File_Shader, Shader_Header});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_scene.glsl", 0, File_Shader, Shader_Scene_Quads});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_resolve_depth_peel.glsl", 0, File_Shader, Shader_ResolveDepthPeel});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_composite_depth_peel.glsl", 0, File_Shader, Shader_CompositeDepthPeel});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_overlay.glsl", 0, File_Shader, Shader_Overlay});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_finalize.glsl", 0, File_Shader, Shader_Finalize});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_overbright.glsl", 0, File_Shader, Shader_Overbright});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_blur.glsl", 0, File_Shader, Shader_Blur});
	RegisterFileHotload(Hotloader,
		{"Shaders/cardcade_debug_view.glsl", 0, File_Shader, Shader_DebugView});
	RegisterFileHotload(Hotloader,
		...
		if(File->NeedsReloading && Win32FileIsReady(File->Filename))
		{
			switch(File->Tag)
			{
				case File_Shader:
				{
					...
					Op.ReloadShader.Type = File->Shader;
                     ...
                     s32 CommandShow)
{
	...
	Win32BuildEXEPathFilename("cardcade.dll", GameCode->DLLName, ArrayCount(GameCode->DLLName));
	Win32BuildEXEPathFilename("cardcade_temp.dll", GameCode->DLLNameTemp, ArrayCount(GameCode->DLLNameTemp));
