/*@H
* File: win32_cardcade_opengl.cpp
* Author: Jesse Calvert
* Created: February 2, 2018, 18:19
* Last modified: November 4, 2018, 18:54
*/
...
#include "win32_cardcade_renderer.h"
...
#include "cardcade_renderer_opengl.h"
#include "cardcade_renderer_opengl.cpp"
...
extern "C" WIN32_LOAD_RENDERER(Win32LoadOpenGLRenderer)
{
	...
	game_render_commands *RenderCommands = &State->RenderCommands;
    RenderCommands->MaxSize = Megabytes(32);
    ...
    umm VertexSize = RenderCommands->MaxVertexCount*sizeof(vertex_format);
    umm IndexSize = RenderCommands->MaxIndexCount*sizeof(u32);
    umm MeshSize = RenderCommands->MaxMeshCount*sizeof(render_command_mesh_data);

	umm TotalStorageSize = RenderCommands->MaxSize +
		VertexSize + IndexSize + MeshSize;

    u8 *Memory = PushSize(&State->Region, TotalStorageSize);

    RenderCommands->Buffer = Memory;
    Memory += RenderCommands->MaxSize;
    RenderCommands->VertexArray = (vertex_format *)Memory;
    Memory += VertexSize;
    RenderCommands->IndexArray = (u32 *)Memory;
    Memory += IndexSize;
	RenderCommands->MeshArray = (render_command_mesh_data *)Memory;
    Memory += MeshSize;
