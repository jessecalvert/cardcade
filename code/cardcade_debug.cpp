/*@H
* File: cardcade_debug.cpp
* Author: Jesse Calvert
* Created: November 6, 2017, 17:31
* Last modified: December 5, 2018, 16:02
*/
	...
	game_input *Input, debug_stored_event *StoredEvent)
{
	...
	switch(Type)
	{
		...
		case Event_AABBTree:
		{
			if(State->GameState->CurrentMode == GameMode_BattleMode)
			{
				DEBUGDisplayAABBTreeDiagram(&State->GameState->BattleMode->Physics.DynamicAABBTree, Group, RectMin, RectMax);
			}
		} break;
...
extern "C" DEBUG_FRAME_END(DebugFrameEnd)
{
	...
	if(GameState)
	{
		...
		if(GameState->CurrentMode == GameMode_BattleMode)
		{
			...
			game_mode_battle *BattleMode = GameState->BattleMode;

			RenderGroup_ = BeginRender(DebugAssets, RenderCommands, Target_Overlay, {}, Render_ClearDepth);
			RenderGroup = &RenderGroup_;
			SetCamera(RenderGroup, &BattleMode->WorldCamera);
			SetRenderFlags(RenderGroup, RenderFlag_NotLighted|RenderFlag_TopMost);

			DEBUGDisplayDynamicAABBTreeRegions(&BattleMode->Physics, RenderGroup);
			DEBUGDisplayStaticAABBTreeRegions(&BattleMode->Physics, RenderGroup);
			DEBUGDisplayContactInfo(&BattleMode->Physics, RenderGroup);
