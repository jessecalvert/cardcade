/*@H
* File: cardcade_ability.cpp
* Author: Jesse Calvert
* Created: November 6, 2018, 16:24
* Last modified: December 17, 2018, 16:53
*/
...
internal entity *
GetOpponent(game_mode_battle *BattleMode, entity *Player)
{
	Assert((Player->Owner == Player_1) || (Player->Owner == Player_2));
	player_name OpponentName = (Player->Owner == Player_1) ? Player_2 : Player_1;
	entity *Opponent = GetEntity(BattleMode->EntityManager, BattleMode->Players[OpponentName]);
	return Opponent;
}
...
internal b32
TileIsTraversable(game_mode_battle *BattleMode, v2i BoardPosition)
{
	b32 Result = true;
	entity_list List = GetEntitiesAtPosition(&BattleMode->Board, BoardPosition);
...
internal ability *
AddAbility(game_mode_battle *BattleMode, entity *Entity, ability_name AbilityName)
{
	Assert(Entity->AbilityCount < ArrayCount(Entity->Abilities));
	ability *Ability = BattleMode->Abilities + AbilityName;
...
internal void
EntityRevertToInfo(game_mode_battle *BattleMode, entity *Entity)
{
	entity_info *Info = Entity->Info;
	if(Info)
	{
		Entity->AbilityCount = 0;
		for(u32 Index = 0;
			Index < Info->AbilityCount;
			++Index)
		{
			ability_name AbilityName = Info->Abilities[Index];
			AddAbility(BattleMode, Entity, AbilityName);
		}
		...
		Entity->Attack = Info->Attack;
		Entity->Defense = Entity->MaxDefense = Info->MaxDefense;
		Entity->Influence = Info->Influence;
		Entity->VictoryPoints = Info->VictoryPoints;
...
internal entity *
CreateEntity(game_mode_battle *BattleMode, entity_manager *Manager, assets *Assets, entity_type Type)
{
	entity *Result = AddEntity(Manager);
	entity_info *Info = GetEntityInfo(Assets, Type);
	Result->Info = Info;
	EntityRevertToInfo(BattleMode, Result);
	return Result;
}

internal void
PlaceEntityOnBoard(game_mode_battle *BattleMode, entity *Entity, v2i BoardPosition)
{
	board_tile *Tile = GetTile(&BattleMode->Board, BoardPosition);
	v3 P = Tile->P;
	v3 Dim = Tile->Dim;
...
internal entity *
CreateEntityOnBoard(game_mode_battle *BattleMode, entity_manager *EntityManager, assets *Assets,
	entity_type Type, v2i BoardPosition, player_name Owner = Player_None)
{
	entity *Result = CreateEntity(BattleMode, BattleMode->EntityManager, Assets, Type);
	PlaceEntityOnBoard(BattleMode, Result, BoardPosition);
	Result->Owner = Owner;
	return Result;
}

internal void
CreateBoardEntityRigidBody(game_state *GameState, game_mode_battle *BattleMode, entity *Entity)
{
	v3 Dim = BattleMode->GroundDim;
	collider Sentinel = {};
	DLIST_INIT(&Sentinel);
	collider *Collider0 = BuildBoxCollider(&GameState->FrameRegion, &BattleMode->Physics,
		V3(0,0,0), Dim, NoRotation());
	DLIST_INSERT(&Sentinel, Collider0);

	move_spec MoveSpec = DefaultMoveSpec();
	InitializeRigidBody(&BattleMode->Physics, Entity, &Sentinel, true, MoveSpec);
}

internal entity *
AddGround(game_state *GameState, game_mode_battle *BattleMode, v2i BoardPosition)
{
	entity *Result = CreateEntityOnBoard(BattleMode, BattleMode->EntityManager, GameState->Assets, Entity_Ground, BoardPosition);
	CreateBoardEntityRigidBody(GameState, BattleMode, Result);
	return Result;
}

internal entity *
AddWater(game_state *GameState, game_mode_battle *BattleMode, v2i BoardPosition)
{
	entity *Result = CreateEntityOnBoard(BattleMode, BattleMode->EntityManager, GameState->Assets, Entity_Water, BoardPosition);
	CreateBoardEntityRigidBody(GameState, BattleMode, Result);
	return Result;
}

internal entity *
AddCard(game_state *GameState, game_mode_battle *BattleMode, player_name Owner, ability_name AbilityName)
{
	entity *Result = CreateEntity(BattleMode, BattleMode->EntityManager, GameState->Assets, Entity_Card);
	Result->Owner = Owner;
	...
	ability *Ability = AddAbility(BattleMode, Result, AbilityName);
...
internal void
DamageEntity(game_state *GameState, game_mode_battle *BattleMode, entity *Entity, s32 Damage)
{
	if((Entity->MaxDefense) && (Entity->Defense > 0))
	{
		...
		if(Entity->Defense <= 0)
		{
				...
				++AbilityIndex)
			{
				...
				{
					entity *Player = GetEntity(BattleMode->EntityManager, BattleMode->Players[Entity->Owner]);
					UseAbility(GameState, BattleMode, Player, Entity, Ability, GameState->UI.TargetIDs);
				}
			}

			if(!(Entity->Flags & EntityFlag_IsBuilding))
			{
				RemoveEntity(BattleMode->EntityManager, Entity);
			}
		}
	}
}
...
internal void
ChargeEntity(game_state *GameState, game_mode_battle *BattleMode, entity *Entity, s32 Charges)
{
...
internal b32
AbilityCostsCanBePaid(game_mode_battle *BattleMode, entity *Player, entity *Entity, ability *Ability)
{
	b32 Result = true;
...
internal void
PayAbilityCosts(game_state *GameState, game_mode_battle *BattleMode,
	entity *Player, entity *Entity, ability *Ability)
{
	ability_cost Cost = Ability->Cost;
	Player->Gold -= Cost.Gold;
	Player->ActionsRemaining -= Cost.Actions;
	ChargeEntity(GameState, BattleMode, Entity, -Cost.Charges);
...
internal b32
PositionIsInfluenced(game_mode_battle *BattleMode, v2i BoardPosition, player_name Player)
{
	b32 Result = false;
...
internal b32
TargetIsValid(game_mode_battle *BattleMode, entity *CastingEntity, ability *Ability, entity *Target)
{
	b32 Result = true;
	...
	{
		if(!TileIsTraversable(BattleMode, Target->BoardPosition))
		{
			Result = false;
		}
	}
	...
	{
		if(!PositionIsInfluenced(BattleMode, Target->BoardPosition, CastingEntity->Owner))
		{
			Result = false;
		}
	}
...
internal void
DrawRandomCard(game_state *GameState, game_mode_battle *BattleMode, entity *Player)
{
	deck *Deck = BattleMode->Decks + Player->Owner;
	u32 Choice = RandomChoice(&BattleMode->Entropy, Deck->CardCount);
	ability_name AbilityName = Deck->Cards[Choice];
	Deck->Cards[Choice] = Deck->Cards[--Deck->CardCount];
	AddCard(GameState, BattleMode, Player->Owner, AbilityName);
}

internal b32
UseAbility(game_state *GameState, game_mode_battle *BattleMode,
	entity *Player, entity *CastingEntity, ability *Ability,
	entity_id *TargetIDs)
{
	b32 AbilityUsed = false;

	b32 CanPayCosts = AbilityCostsCanBePaid(BattleMode, Player, CastingEntity, Ability);
	if(CanPayCosts)
	{
		...
		for(u32 TargetIndex = 0;
			...
			++TargetIndex)
		{
			Targets[TargetIndex] = GetEntity(BattleMode->EntityManager, TargetIDs[TargetIndex]);
		}

		PayAbilityCosts(GameState, BattleMode, Player, CastingEntity, Ability);
		AbilityUsed = true;
		...
		switch(Ability->Name)
		{
			case Ability_Move:
			{
				...
				Assert(TileIsTraversable(BattleMode, BoardPosition));

				PushInterpAnimation(&BattleMode->Animator, CastingEntity, P - CastingEntity->P,
			...
			{
				v3 P = Targets[0]->P;
				SpawnFire(BattleMode->ParticleSystem, P);

				entity_list List = GetEntitiesAtPosition(&BattleMode->Board, Targets[0]->BoardPosition);
				for(u32 EntityIndex = 0;
					EntityIndex < List.EntityCount;
					++EntityIndex)
				{
					entity *Entity = GetEntity(BattleMode->EntityManager, List.EntityIDs[EntityIndex]);
					DamageEntity(GameState, BattleMode, Entity, CastingEntity->Attack);
				}
			} break;

			case Ability_GenerateCharge:
			{
				ChargeEntity(GameState, BattleMode, CastingEntity, Ability->Parameters[0]);
			} break;

			case Ability_BuildHeadquarters:
			{
				v2i BoardPosition = Targets[0]->BoardPosition;
				CreateEntityOnBoard(BattleMode, BattleMode->EntityManager, GameState->Assets,
			...
			{
				v2i BoardPosition = Targets[0]->BoardPosition;
				entity *Bank = CreateEntityOnBoard(BattleMode, BattleMode->EntityManager, GameState->Assets,
			...
			case Ability_BankPillaged:
			{
				...
				{
					ChargeEntity(GameState, BattleMode, CastingEntity, -1);
					entity *Opponent = GetOpponent(BattleMode, Player);
			...
			{
				v2i BoardPosition = Targets[0]->BoardPosition;
				entity *RiskInvestment = CreateEntityOnBoard(BattleMode, BattleMode->EntityManager,
			...
			case Ability_RiskInvestmentReturn:
			{
				if(CastingEntity->Charges >= Ability->Parameters[0])
				{
					...
					RemoveEntity(BattleMode->EntityManager, CastingEntity);
				}
			} break;
			...
			{
				v2i BoardPosition = Targets[0]->BoardPosition;
				entity *GoldMine = CreateEntityOnBoard(BattleMode, BattleMode->EntityManager, GameState->Assets,
			...
			{
				v2i BoardPosition = Targets[0]->BoardPosition;
				entity *PowerPlant = CreateEntityOnBoard(BattleMode, BattleMode->EntityManager, GameState->Assets, Entity_PowerPlant, BoardPosition, Player->Owner);
			} break;
			...
			case Ability_GeneratePower:
			{
				...
				FOR_EACH(Entity, BattleMode->EntityManager, entity)
				{
					...
					{
						ChargeEntity(GameState, BattleMode, Entity, Ability->Parameters[1]);
					}
				}
			} break;
			...
			{
				v3 P = Targets[0]->P;
				SpawnFire(BattleMode->ParticleSystem, P, 20);
				...
				{
					entity *Entity = GetEntity(BattleMode->EntityManager, List.EntityIDs[EntityIndex]);
					DamageEntity(GameState, BattleMode, Entity, Ability->Parameters[0]);
				}
			} break;
			...
			{
				v2i BoardPosition = Targets[0]->BoardPosition;
				entity *Mercs = CreateEntityOnBoard(BattleMode, BattleMode->EntityManager, GameState->Assets, Entity_Mercenaries, BoardPosition, Player->Owner);
			} break;
